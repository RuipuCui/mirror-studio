    ...
    <section className="py-24 px-6 relative border-t border-black/5 overflow-hidden">
      <div className="max-w-7xl mx-auto text-center">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h2 className="text-4xl md:text-5xl font-bold text-black mb-6">Human OS Installer</h2>
          <p className="text-xl text-slate-500 mb-16 max-w-2xl mx-auto">
            Mirror Academy 是 WhiteMirror 的「人类操作系统安装器」，为整个生态输送能驾驭这一切的人。
          </p>
          ...
          <div className="relative flex flex-col items-center">
            ...
            <div className="relative z-20 w-48 h-48 rounded-full bg-black text-white flex flex-col items-center justify-center shadow-2xl shadow-black/30 mb-12">
              <div className="text-3xl font-bold mb-1">Academy</div>
              <div className="text-xs opacity-70 uppercase tracking-wider">Talent Engine</div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-12 w-full max-w-4xl">
              {[
                { icon: FlaskConical, label: "Lab", desc: "发现方向" },
                { icon: Layout, label: "Studio", desc: "实现系统" },
                { icon: Database, label: "Research", desc: "抽象方法" },
                { icon: Share2, label: "Post", desc: "传播叙事" },
              ].map((item, i) => (
                <motion.div
                  key={i}
                  initial={{ opacity: 0, y: 20 }}
